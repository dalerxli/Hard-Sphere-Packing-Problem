...
namespace kspace {
	...
	public:
		sim(double end_time, double time_step_size, double initial_diameter, double Growth_Rate, int width, int height, int depth, double cell_width, double cell_height, double cell_depth); //initialises the sim
		~sim();
...
sim::sim(double end_time, double time_step_size, double initial_diameter, double Growth_Rate, int width, int height, int depth, double cell_width, double cell_height, double cell_depth) {
	current_time = 0;
	previous_time = 0;
...
void sim::run() {
	...
	do {
		...
		if(event_q.size() > 0) {
			...
			if(st == 0) {
				...
				ft = clock();
				double realt = (double) (ft - st);
				double simt = (current_time - previous_time)*1000;
				cout << "sim: "<< simt << " | real: " << realt << " | sim/real = " << simt / realt << endl;
	...
	make_concurrent();
	cout << "Packing Fraction = " << 4*4*atan(1.)*pow((init_diameter() + growth_rate()*current_time)/2, 3) / 3 << endl;
...
void sim::advance(cEvent *ev) {
	...
	for(int a = 0; a < sphere_count; a++) {
		...
		//advance the sphere to the currentd time
	...
	if(valid_event) {
		...
		if(sphere_count == 1) {
			...
			for(int a = 1; a < sphere_count; a++) {
				...
				get_sphere(id2[0], id2[1], id2[2]).inc_events();
			}
			get_sphere(id1[0], id1[1], id1[2]).inc_events();
			sev_counter++;
		}

		//calculate the next event for all spheres involved in this event
		for(int a = 0; a < sphere_count; a++) {
			id1 = ev->get_id(a);
		...
		for(int a = 0; a < sphere_count; a++) {
			id1 = ev->get_id(a);
			if(get_sphere(id1[0], id1[1], id1[2]).get_event_count() - ev->get_event_count(a) == 0) {
				id2 = get_sphere(id1[0], id1[1], id1[2]).get_id(); //get the actual id
				calculate_events(id2[0], id2[1], id2[2]); //calculate the new events for the sphere
			}
		}
...
double sim::init_diameter() const {
	return D0;
}
...
void sim::calculate_events(const int i, const int j, const int k) {
	...
	if(bev >= 0) {
			...
			for(int b = s.get_lower_clim(1) - 1; b <= s.get_upper_clim(1); b++) {
				for(int c = s.get_lower_clim(2) - 1; c <= s.get_upper_clim(2); c++) {
					if(a != i || b != j || c != k) {
						...
						if(sev >= 0) {
							//compare the new event time with the previous minimum and pick the minimum one
							if(ev->get_time() < 0) {
								ev->set_time(sev);
								ev->add_sphere(a, b, c, get_sphere(a, b, c).get_event_count());
							} else {
								if(round(ev->get_time() - sev) == 0) {
									//if the newly calculated event occurs at the same time as a previous event add the new sphere
									//to the event
									ev->add_sphere(a, b, c, get_sphere(a, b, c).get_event_count());
								} else if (ev->get_time() > sev) {
									//if the newly calculated event occurs sooner remove all previous spheres (except for the first one)
									//and add the new one
									ev->reset();
									ev->set_time(sev);
									ev->add_sphere(a, b, c, get_sphere(a, b, c).get_event_count());
		...
		if((bev < ev->get_time() || ev->get_time() < 0) && bev <= step_time) {
			delete ev;
			if(bev <= step_time) {
		...
		} else if(ev->get_time() >= 0 && ev->get_time() <= step_time) {
			//add the sphere-sphere collision to the queue
			event_q.push(ev);
		} else {
			delete ev;
		}
...
double sim::calculate_sphere_event(const int i, const int j, const int k, const int l, const int m, const int n) {
	...
	double p[3], v[3], np[3], V;
	for(int a = 0; a < 3; a++) {
		p[a] = advPos2[a] - advPos1[a];
		v[a] = s2.get_vel(a) - s1.get_vel(a);
	}

	double P = p[0]*p[0] + p[1]*p[1] + p[2]*p[2];
	...
	double D_max_t = init_diameter() + growth_rate()*max_t; //the diameter of the spheres at t(maximum)
	...
	if((dN >= 0 && V < growth_rate()) || (dN < 0 && V > 0)) {
		...
		C = p[0]*p[0] + p[1]*p[1] + p[2]*p[2] - D_max_t*D_max_t; //coefficient of the t^0 term;
...
//print arrays
void sim::print(double *arr, int size) {
	cout << "[";
